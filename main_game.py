...
print_event("⚔️   Entering the dungeon... ⚔️")
# -------------------------------------------------------------- #
#                       -> START MENU <-                         #
# -------------------------------------------------------------- #
...
print_event("Starting game...", 
            f"Level {run_data.floor.level} - {run_data.floor.name}", run_data.floor.description)
print("\n---------------------------------------------------------------")
...
while run_data.floor.level < 10:  # FLoor levels control
    ...
    # Rest place event
        ...
        if character.character_class == "Mage":
            if character.hp < character.get_full_hp() or character.mp < character.get_full_mp():
                canRest = True
        else:
            if character.hp < character.get_full_hp() or character.stamina < character.get_full_stamina():
                canRest = True
